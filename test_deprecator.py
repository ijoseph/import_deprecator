...
class TestDeprecator(unittest.TestCase):
    ...
    def test_deprecate(self):
        ...
        with warnings.catch_warnings():
            ...
            from example_library_module import replacement

        # 'bar' is deprecated, warns
        with warnings.catch_warnings(record=True) as w:
            from example_library_module import previous_name

            self.assertTrue(len(w) > 0)
            self.assertIn(
                "'example_library_module.previous_name' has been renamed", str(w[0])
            )
